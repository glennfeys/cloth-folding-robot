...
namespace SoftBody
{
    public sealed partial class RectangularCloth : ClothSimulation
    {
        ...
        /// <summary>
        /// Perform an artificial fold and rotate
        /// </summary>
        public void MakeFold()
        {
            var middle = SpringProcessor.SpringNodeFor(CoordToIndex(0, amountOfDivisionsInMesh / 2)).Position.z;
            for (var z = amountOfDivisionsInMesh / 2; z <= amountOfDivisionsInMesh; z++)
            for (var x = 0; x <= amountOfDivisionsInMesh; x++)
            {
                var springNode = SpringProcessor.SpringNodeFor(CoordToIndex(x, z));
                var springNodePosition = springNode.Position;
                springNodePosition.y += 2f / amountOfDivisionsInMesh;
                springNodePosition.z = (springNodePosition.z - middle) * -1 + middle;
                springNode.SnapTo(springNodePosition, false);
            }
